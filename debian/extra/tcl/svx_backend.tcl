...
namespace eval ::SVX {
  # re-source safe init
  variable DEBUG;             if {![info exists ::SVX::DEBUG]} { set DEBUG 0 }
  variable RAM_DIR;           if {![info exists ::SVX::RAM_DIR]} { set RAM_DIR "/run/svxlink" }
  ...
  variable LOCK_DIR;          if {![info exists ::SVX::LOCK_DIR]} { set LOCK_DIR "/run/lock/svxlink" }
  variable HISTORY_LOCK;      if {![info exists ::SVX::HISTORY_LOCK]} { set HISTORY_LOCK "$LOCK_DIR/history.lock" }
  variable CSV_HEADER;        if {![info exists ::SVX::CSV_HEADER]} { set CSV_HEADER "timestamp;src;event;data" }
  variable QUEUE_FILE;        if {![info exists ::SVX::QUEUE_FILE]} { set QUEUE_FILE "$RAM_DIR/history.queue" }
...
proc ::SVX::ensure_dirs {} {
  ...
  if {![file isdirectory $::SVX::LOCK_DIR]} { catch { file mkdir $::SVX::LOCK_DIR } }
}
...
proc ::SVX::ensure_node_defaults {n} {
  foreach {k def} {
    isTalker 0 tg 0 last_seen 0 last_talk_start 0 last_talk_stop 0
...
proc ::SVX::__read_body {path} {
  set lines [::SVX::__read_lines $path]
  if {[llength $lines] && [lindex $lines 0] eq $::SVX::CSV_HEADER} {
    return [lrange $lines 1 end]
  }
  return $lines
}
proc ::SVX::__ensure_csv {path} {
  if {![file exists $path] || [file size $path]==0} {
    ::SVX::__atomic_write $path "$::SVX::CSV_HEADER\n"
  } else {
    set lines [::SVX::__read_lines $path]
    if {![llength $lines] || [lindex $lines 0] ne $::SVX::CSV_HEADER} {
      set body [join $lines \n]
      ::SVX::__atomic_write $path "$::SVX::CSV_HEADER\n$body\n"
    }
  }
...
proc ::SVX::__trylock {} {
  ::SVX::ensure_dirs
  if {[catch { set fd [open $::SVX::HISTORY_LOCK {WRONLY CREAT EXCL}] }]} { return "" }
  fconfigure $fd -translation binary
  puts -nonewline $fd [pid]; flush $fd
  return $fd
}
proc ::SVX::__unlock {fd} {
  if {$fd ne ""} { catch { close $fd } }
  catch { file delete -force $::SVX::HISTORY_LOCK }
}
proc ::SVX::__enqueue_line {line} {
  ::SVX::ensure_dirs
  set f [open $::SVX::QUEUE_FILE a]; fconfigure $f -translation binary -encoding utf-8
  puts $f $line; close $f
}
...
proc ::SVX::append_history {name tg dur {start_ts {}}} {
  ...
  set ts [clock format $start_ts -format "%d.%m.%Y %H:%M:%S"]
  set line "$ts;$name;[::SVX::num $tg];[::SVX::num $dur]"
  set fd [::SVX::__trylock]
  if {$fd eq ""} {
    ::SVX::__enqueue_line $line
    ::SVX::dbg "history queued (lock busy)"
    return
  }
  try {
    ::SVX::ensure_dirs
    ::SVX::__ensure_csv $::SVX::HISTORY
    ::SVX::__ensure_csv $::SVX::DISK_FILE
    set qlines {}
    if {[file exists $::SVX::QUEUE_FILE]} {
      set qlines [::SVX::__read_lines $::SVX::QUEUE_FILE]
      catch { file delete -force $::SVX::QUEUE_FILE }
    }
    set body [::SVX::__read_body $::SVX::HISTORY]
    set merged [concat $body $qlines [list $line]]
    set keep $::SVX::HISTORY_LIMIT
    if {[llength $merged] > $keep} { set merged [lrange $merged end-[expr {$keep-1}] end] }
    set NL [format %c 10]
    set data "$::SVX::CSV_HEADER$NL"
    if {[llength $merged]} { append data "[join $merged $NL]$NL" }
    ::SVX::__atomic_write $::SVX::HISTORY $data
    foreach l $qlines { catch { ::SVX::__append_line $::SVX::DISK_FILE $l } }
    catch { ::SVX::__append_line $::SVX::DISK_FILE $line }
    ::SVX::dbg "history+1 (node=$name tg=$tg dur=$dur) size=[llength $merged]"
  } finally {
    ::SVX::__unlock $fd
  }
...
if {[info procs SimplexLogic::every_second] eq ""} {
    proc SimplexLogic::every_second {} {}
}
if {[info procs SimplexLogic::every_minute] eq ""} {
